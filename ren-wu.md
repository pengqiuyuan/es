...
【1】单个监测项任务计算结果的保存（Es或者mongodb）。每条文档都有，词包父ID ，词包子ID，监测项名称，起始日期、结束日期，指标属于类别（微博、微信等等）
...
```
传入参数：监测项名称，起始日期、结束日期，指标属于类别

1、查询次任务表，根据监测项判断，一定时间范围内是否已经存在mongodb（或者es？）并且已经执行完成指标值。
